...
if __name__ == "__main__":
    ...
    # ---- Plot one series (clean axis & 1-year history) ----
# ---- Robust plot: one year of history, clean axes ----# ---- Plot CV: history + true + predicted (robust) ----
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt

# 1) find CV dataframe
for name in ['Y_hat_df','Y_hat_X','Y_hat2','Y_hat_noX','Y_hat']:
    if name in globals():
        cv = globals()[name]
        break
else:
    raise RuntimeError("No CV dataframe found (e.g., Y_hat_df). Run cross_validation first.")

# 2) pick prediction column
base = {'unique_id','ds','y','cutoff','split','model'}
cands = [c for c in ['NHITS','AutoNHITS'] if c in cv.columns]
if cands:
    pred_col = cands[0]
else:
    num_cols = [c for c in cv.columns
                if c not in base and np.issubdtype(cv[c].dtype, np.number)]
    if not num_cols:
        raise RuntimeError(f"No numeric prediction column found in columns={list(cv.columns)}")
    pred_col = num_cols[0]
print(f"[plot] Using prediction column: {pred_col}")

# 3) filter to first series + windowing
first_id = Y_df['unique_id'].iloc[0]
fh = cv[cv['unique_id'] == first_id].copy()
if fh.empty:
    raise RuntimeError("CV dataframe has no rows for the first series.")

first_fcst_start = fh['ds'].min()
hist_start = max(pd.Timestamp("2025-05-01"),
                 first_fcst_start - pd.to_timedelta(52, unit="W"))

history = Y_df[(Y_df['unique_id'] == first_id) &
               (Y_df['ds'] >= hist_start) &
               (Y_df['ds'] <= first_fcst_start)].copy()

# 4) plot
fig, ax = plt.subplots(figsize=(12,6))
ax.plot(history['ds'], history['y'], color="black", label="History (last 52 weeks)")
ax.plot(fh['ds'], fh['y'], color="green", label="True (CV windows)")
ax.plot(fh['ds'], fh[pred_col], '--', linewidth=1.8, color="blue",
        label=f"Predicted ({pred_col})")

# axes, labels, limits
ax.set_xlim(pd.Timestamp("2025-05-01"), pd.Timestamp("2025-10-31"))
ax.set_ylim(250, 350)
ax.set_xlabel("Time")
ax.set_ylabel("Tea Price (rupees)")
ax.set_title("NHITS Forecast (weekly, h=12)")
ax.grid(True); ax.legend(loc="best")
fig.tight_layout(); plt.show()

# ---------- Residuals (CV only): True - Pred ----------
fh = cv[cv['unique_id'] == first_id].copy()
fh['resid'] = fh['y'] - fh[pred_col]

# quick stats (bias, MAE, RMSE)
print(f"Residual bias (mean): {fh['resid'].mean():.3f}")
print(f"Residual MAE: {np.abs(fh['resid']).mean():.3f}")
print(f"Residual RMSE: {np.sqrt((fh['resid']**2).mean()):.3f}")

# Residuals over time
fig, ax = plt.subplots(figsize=(12,4))
ax.plot(fh['ds'], fh['resid'])
ax.axhline(0, linestyle='--', linewidth=1)
ax.set_xlim(pd.Timestamp('2025-05-01'), pd.Timestamp('2025-10-31'))  # match your main plot window
ax.set_xlabel("Time")
ax.set_ylabel("Residual (rupees)")   # True - Pred
ax.set_title("NHITS Residuals over Time (CV)")
ax.grid(True)
fig.tight_layout()
plt.show()

# Residual distribution (optional but useful)
fig, ax = plt.subplots(figsize=(6,4))
ax.hist(fh['resid'].dropna(), bins=15)
ax.set_xlabel("Residual (rupees)")
ax.set_ylabel("Count")
ax.set_title("Residual Histogram (CV)")
fig.tight_layout()
plt.show()

# Residual autocorrelation (optional; skip if statsmodels not installed)
try:
    from statsmodels.tsa.stattools import acf
    ac = acf(fh['resid'].dropna(), nlags=12, fft=True)
    fig, ax = plt.subplots(figsize=(6,4))
    ax.stem(range(len(ac)), ac, use_line_collection=True)
    ax.set_xlabel("Lag (weeks)")
    ax.set_ylabel("ACF")
    ax.set_title("Residual ACF (CV)")
    fig.tight_layout()
    plt.show()
except Exception:
    pass